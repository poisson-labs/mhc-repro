...
class ResidualConnection(nn.Module):
    ...
    For residual connections, we DON'T use expanded streams.
    Input and output are always (batch, seq, hidden_dim).
    """
    ...
    def __init__(self, hidden_dim: int, expansion_rate: int = 4, **kwargs):
        ...
        self.uses_streams = False  # Flag to indicate no stream expansion
...
class HyperConnection(nn.Module):
    ...
    CRITICAL: This class now expects PRE-EXPANDED input (n, batch, seq, dim)
    and returns (n, batch, seq, dim). Stream expansion/collapse happens at
    the GPT model level, not within each connection.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.uses_streams = True  # Flag to indicate stream expansion is used
    ...
    def forward(
        self, z: torch.Tensor, sublayer: Callable[[torch.Tensor], torch.Tensor]
    ) -> torch.Tensor:
        """
        Args:
            z: Input tensor of shape (n, batch, seq, hidden_dim) - PRE-EXPANDED streams
            sublayer: Function F to apply (expects (batch, seq, dim), returns same)

        Returns:
            Output tensor of shape (n, batch, seq, hidden_dim) - streams persist
        """
        n, batch, seq, dim = z.shape
        ...
        # Apply sublayer (operates on collapsed (B,S,D) tensor)
        ...
        # Add sublayer contribution - streams PERSIST
        z_new = z_mixed + delta

        return z_new  # Return ALL streams, not just first
...
if __name__ == "__main__":
    ...
    # Simple sublayer for testing (expects (B,S,D), returns (B,S,D))
    def dummy_sublayer(x):
        return torch.tanh(x)

    # Test Residual (uses (B,S,D))
    print("\nResidual:")
    x_residual = torch.randn(batch, seq, dim)
    conn_residual = ResidualConnection(hidden_dim=dim, expansion_rate=n)
    y_residual = conn_residual(x_residual, dummy_sublayer)
    print(f"  Input shape: {x_residual.shape}")
    print(f"  Output shape: {y_residual.shape}")
    print(f"  uses_streams: {conn_residual.uses_streams}")

    # Test HC and MHC (now use (n,B,S,D))
    for name, cls in [("HC", HyperConnection), ("mHC", ManifoldHyperConnection)]:
        conn = cls(hidden_dim=dim, expansion_rate=n)
        # Create expanded input (n, B, S, D)
        z = torch.randn(n, batch, seq, dim)
        y = conn(z, dummy_sublayer)

        print(f"\n{name}:")
        print(f"  Input shape: {z.shape}")
        print(f"  Output shape: {y.shape}")
        print(f"  uses_streams: {conn.uses_streams}")
    ...
    for name, cls in [("HC", HyperConnection), ("mHC", ManifoldHyperConnection)]:
        conn = cls(hidden_dim=dim, expansion_rate=n)
        z = torch.randn(n, batch, seq, dim, requires_grad=True)
        y = conn(z, dummy_sublayer)
        loss = y.sum()
        loss.backward()

        print(f"\n{name}:")
        print(f"  Gradient through z: {z.grad is not None}")
    ...
    for name, cls in [("HC", HyperConnection), ("mHC", ManifoldHyperConnection)]:
        ...
        # Test with two different inputs (both (n, B, S, D))
        z1 = torch.randn(n, batch, seq, dim)
        z2 = torch.randn(n, batch, seq, dim) * 2 + 1  # Different input

        # Forward pass with first input
        _ = conn(z1, dummy_sublayer)
        h_res_1 = conn.get_h_res().clone()

        # Forward pass with second input
        _ = conn(z2, dummy_sublayer)
    ...
    # Test stream persistence (CRITICAL for the fix)
    print("\n" + "=" * 50)
    print("Testing STREAM PERSISTENCE (critical test)...")
    ...
    for name, cls in [("HC", HyperConnection), ("mHC", ManifoldHyperConnection)]:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                diff = (y[i] - y[j]).abs().mean().item()
                stream_diffs.append(diff)

        avg_stream_diff = sum(stream_diffs) / len(stream_diffs)
        streams_diverged = avg_stream_diff > 1e-3

        print(f"\n{name}:")
        print(f"  Avg stream difference: {avg_stream_diff:.6f}")
        print(f"  STREAMS DIVERGED: {'YES' if streams_diverged else 'NO (FAIL)'}")

        if not streams_diverged:
            raise AssertionError(f"{name}: Streams did NOT diverge!")
